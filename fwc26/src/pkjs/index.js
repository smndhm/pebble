...
var teamNames = {
  fr: [
    ...
    'Brésil', 'Canada', 'Cap-Vert', 'Colombie', 'Corée du Sud',
    "Côte d'Ivoire", 'Croatie', 'Curaçao', 'Écosse', 'Égypte',
    'Équateur', 'Espagne', 'États-Unis', 'France', 'Ghana',
    'Haïti', 'Irak', 'Iran', 'Japon', 'Jordanie',
    'Maroc', 'Mexique', 'Norvège', 'Nouvelle-Zélande', 'Ouzbékistan',
    'Panama', 'Paraguay', 'Pays-Bas', 'Portugal', 'Qatar',
    'RD Congo', 'Sénégal', 'Suède', 'Suisse', 'Tchéquie',
    'Tunisie', 'Türkiye', 'Uruguay',
  ],
  en: [
    'South Africa', 'Algeria', 'Germany', 'England', 'Saudi Arabia',
    'Argentina', 'Australia', 'Austria', 'Belgium', 'Bosnia and Herzegovina',
    'Brazil', 'Canada', 'Cape Verde', 'Colombia', 'South Korea',
    "Côte d'Ivoire", 'Croatia', 'Curaçao', 'Scotland', 'Egypt',
    'Ecuador', 'Spain', 'United States', 'France', 'Ghana',
    'Haiti', 'Iraq', 'Iran', 'Japan', 'Jordan',
    'Morocco', 'Mexico', 'Norway', 'New Zealand', 'Uzbekistan',
    'Panama', 'Paraguay', 'Netherlands', 'Portugal', 'Qatar',
    'DR Congo', 'Senegal', 'Sweden', 'Switzerland', 'Czechia',
    'Tunisia', 'Türkiye', 'Uruguay',
  ],
};
...
var langCode = (typeof navigator !== 'undefined' && navigator.language ? navigator.language : 'en').split('-')[0].toLowerCase();
...
var countryToTeam = {
  ZA:  0, DZ:  1, DE:  2, GB:  3, SA:  4, AR:  5, AU:  6, AT:  7,
  BE:  8, BA:  9, BR: 10, CA: 11, CV: 12, CO: 13, KR: 14, CI: 15,
  HR: 16, CW: 17,         EG: 19, EC: 20, ES: 21, US: 22, FR: 23,
  GH: 24, HT: 25, IQ: 26, IR: 27, JP: 28, JO: 29, MA: 30, MX: 31,
  NO: 32, NZ: 33, UZ: 34, PA: 35, PY: 36, NL: 37, PT: 38, QA: 39,
  CD: 40, SN: 41, SE: 42, CH: 43, CZ: 44, TN: 45, TR: 46, UY: 47,
};
...
var clayConfig = [
  ...
  },
  {
    ...
    items: [
      ...
      { type: 'color', messageKey: 'BGCOLOR',      label: t.bg,        allowGray: false, defaultValue: '000000' },
      { type: 'color', messageKey: 'FGCOLOR',      label: t.left,      allowGray: false, defaultValue: 'FFFFFF' },
      { type: 'color', messageKey: 'COLOR_RIGHT',  label: t.right,     allowGray: false, defaultValue: 'FFFFFF' },
      { type: 'color', messageKey: 'COL_COLOR',    label: t.collision, allowGray: false, defaultValue: '000000' },
    ],
  },
...
function clayCustomFn() {
  ...
  clay.on(clay.EVENTS.AFTER_BUILD, function() {
    var watchInfo = (typeof Pebble !== 'undefined' && Pebble.getActiveWatchInfo) ? Pebble.getActiveWatchInfo() : {};
    var isBW = (watchInfo.platform === 'aplite' || watchInfo.platform === 'diorite' || watchInfo.platform === 'flint');
    ...
    if (teamItem) {
      ...
      if (tIdx > 0 && all[tIdx - 1].config && all[tIdx - 1].config.type === 'heading') {
        teamHeading = all[tIdx - 1];
      }
    }

    function getMode()    { return modeItem  ? parseInt(modeItem.get(),  10) : 0; }
    function getPixel()   { return pixelItem ? parseInt(pixelItem.get(), 10) : 0; }
    function getTeamIdx() { return teamItem  ? parseInt(teamItem.get(),  10) : 48; }
    ...
    function update() {
      ...
      if (isBW) {
        ...
        [teamItem, teamHeading].forEach(function(it) { if (it) it.hide(); });
        [bgItem, fgItem, rightItem, colItem].forEach(function(it) { if (it) it.hide(); });
    ...
    s_applyWatchConfig = function() {
      ...
      if (teamItem    && s_watchConfig.TEAM_INDEX   !== undefined) teamItem.set(s_watchConfig.TEAM_INDEX);
      if (pixelItem   && s_watchConfig.LOGO_PIXEL   !== undefined) pixelItem.set(s_watchConfig.LOGO_PIXEL);
      if (subModeItem && s_watchConfig.SUB_MODE     !== undefined) subModeItem.set(s_watchConfig.SUB_MODE);
      if (modeItem    && s_watchConfig.DISPLAY_MODE !== undefined) modeItem.set(s_watchConfig.DISPLAY_MODE);
      update();
    };
    ...
    s_watchConfig = null;
    requestConfig();
    setTimeout(function() { if (!s_watchConfig) update(); }, 3000);
      ...
      navigator.geolocation.getCurrentPosition(function(pos) {
        ...
        xhr.onreadystatechange = function() {
          ...
          try {
            ...
            var idx  = (cc === 'GB') ? (sub === 'GB-SCT' ? 18 : 3) : countryToTeam[cc];
            if (idx !== undefined) { teamItem.set(idx); update(); }
          } catch (e) {}
        };
...
function requestConfig() {
  Pebble.sendAppMessage({ REQUEST_CONFIG: 1 }, function() {}, function() {});
}
